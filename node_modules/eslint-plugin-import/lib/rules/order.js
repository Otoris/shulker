...
function reverse(array) ***REMOVED***
  return array.map(function (v) ***REMOVED***
    return ***REMOVED***
      name: v.name,
      rank: -v.rank,
      node: v.node
...
function getTokensOrCommentsAfter(sourceCode, node, count) ***REMOVED***
  let currentNodeOrToken = node;
  const result = [];
  for (let i = 0; i < count; i++) ***REMOVED***
    currentNodeOrToken = sourceCode.getTokenOrCommentAfter(currentNodeOrToken);
    if (currentNodeOrToken == null) ***REMOVED***
      break;
    ***REMOVED***
...
function getTokensOrCommentsBefore(sourceCode, node, count) ***REMOVED***
  let currentNodeOrToken = node;
  const result = [];
  for (let i = 0; i < count; i++) ***REMOVED***
...
function takeTokensAfterWhile(sourceCode, node, condition) ***REMOVED***
  const tokens = getTokensOrCommentsAfter(sourceCode, node, 100);
  const result = [];
  for (let i = 0; i < tokens.length; i++) ***REMOVED***
    if (condition(tokens[i])) ***REMOVED***
...
function takeTokensBeforeWhile(sourceCode, node, condition) ***REMOVED***
...
function findOutOfOrder(imported) ***REMOVED***
  if (imported.length === 0) ***REMOVED***
    return [];
  ***REMOVED***
  let maxSeenRankNode = imported[0];
  return imported.filter(function (importedModule) ***REMOVED***
    const res = importedModule.rank < maxSeenRankNode.rank;
    if (maxSeenRankNode.rank < importedModule.rank) ***REMOVED***
...
function findRootNode(node) ***REMOVED***
  let parent = node;
  while (parent.parent != null && parent.parent.body == null) ***REMOVED***
    parent = parent.parent;
  ***REMOVED***
  return parent;
***REMOVED***

function findEndOfLineWithComments(sourceCode, node) ***REMOVED***
  ...
  for (let i = endOfTokens; i < sourceCode.text.length; i++) ***REMOVED***
    if (sourceCode.text[i] === '\n') ***REMOVED***
      result = i + 1;
      break;
    ***REMOVED***
    if (sourceCode.text[i] !== ' ' && sourceCode.text[i] !== '\t' && sourceCode.text[i] !== '\r') ***REMOVED***
      break;
    ***REMOVED***
    result = i + 1;
  ***REMOVED***
...
function commentOnSameLineAs(node) ***REMOVED***
...
function findStartOfLineWithComments(sourceCode, node) ***REMOVED***
  const tokensToEndOfLine = takeTokensBeforeWhile(sourceCode, node, commentOnSameLineAs(node));
  let startOfTokens = tokensToEndOfLine.length > 0 ? tokensToEndOfLine[0].range[0] : node.range[0];
  let result = startOfTokens;
  for (let i = startOfTokens - 1; i > 0; i--) ***REMOVED***
    if (sourceCode.text[i] !== ' ' && sourceCode.text[i] !== '\t') ***REMOVED***
      break;
    ***REMOVED***
    result = i;
  ***REMOVED***
...
function isPlainRequireModule(node) ***REMOVED***
...
function isPlainImportModule(node) ***REMOVED***
  return node.type === 'ImportDeclaration' && node.specifiers != null && node.specifiers.length > 0;
***REMOVED***

function canCrossNodeWhileReorder(node) ***REMOVED***
  return isPlainRequireModule(node) || isPlainImportModule(node);
***REMOVED***

function canReorderItems(firstNode, secondNode) ***REMOVED***
  const parent = firstNode.parent;
  const firstIndex = parent.body.indexOf(firstNode);
  const secondIndex = parent.body.indexOf(secondNode);
  const nodesBetween = parent.body.slice(firstIndex, secondIndex + 1);
  for (var nodeBetween of nodesBetween) ***REMOVED***
    if (!canCrossNodeWhileReorder(nodeBetween)) ***REMOVED***
      return false;
    ***REMOVED***
  ***REMOVED***
  return true;
...
function fixOutOfOrder(context, firstNode, secondNode, order) ***REMOVED***
  ...
  ***REMOVED***
  ...
  if (order === 'before') ***REMOVED***
    ...
    ***REMOVED***);
  ***REMOVED*** else if (order === 'after') ***REMOVED***
    context.report(***REMOVED***
      node: secondNode.node,
      message: message,
...
function reportOutOfOrder(context, imported, outOfOrder, order) ***REMOVED***
  outOfOrder.forEach(function (imp) ***REMOVED***
    const found = imported.find(function hasHigherRank(importedItem) ***REMOVED***
...
function makeOutOfOrderReport(context, imported) ***REMOVED***
  const outOfOrder = findOutOfOrder(imported);
  if (!outOfOrder.length) ***REMOVED***
    return;
  ***REMOVED***
  // There are things to report. Try to minimize the number of reported errors.
  const reversedImported = reverse(imported);
  const reversedOrder = findOutOfOrder(reversedImported);
  if (reversedOrder.length < outOfOrder.length) ***REMOVED***
...
function computeRank(context, ranks, name, type) ***REMOVED***
  return ranks[(0, _importType2.default)(name, context)] + (type === 'import' ? 0 : 100);
***REMOVED***

function registerNode(context, node, name, type, ranks, imported) ***REMOVED***
  const rank = computeRank(context, ranks, name, type);
  if (rank !== -1) ***REMOVED***
    imported.push(***REMOVED*** name, rank, node ***REMOVED***);
  ***REMOVED***
***REMOVED***

function isInVariableDeclarator(node) ***REMOVED***
...
// Example: ***REMOVED*** index: 0, sibling: 1, parent: 1, external: 1, builtin: 2, internal: 2 ***REMOVED***
// Will throw an error if it contains a type that does not exist, or has a duplicate
function convertGroupsToRanks(groups) ***REMOVED***
  const rankObject = groups.reduce(function (res, group, index) ***REMOVED***
    if (typeof group === 'string') ***REMOVED***
      group = [group];
    ***REMOVED***
    group.forEach(function (groupItem) ***REMOVED***
      if (types.indexOf(groupItem) === -1) ***REMOVED***
        throw new Error('Incorrect configuration of the rule: Unknown type `' + JSON.stringify(groupItem) + '`');
      ***REMOVED***
      if (res[groupItem] !== undefined) ***REMOVED***
  ...
  ***REMOVED***, ***REMOVED******REMOVED***);
  ...
  const omittedTypes = types.filter(function (type) ***REMOVED***
    return rankObject[type] === undefined;
  ***REMOVED***);

  return omittedTypes.reduce(function (res, type) ***REMOVED***
    res[type] = groups.length;
    return res;
  ***REMOVED***, rankObject);
***REMOVED***

function fixNewLineAfterImport(context, previousImport) ***REMOVED***
  const prevRoot = findRootNode(previousImport.node);
  const tokensToEndOfLine = takeTokensAfterWhile(context.getSourceCode(), prevRoot, commentOnSameLineAs(prevRoot));

  let endOfLine = prevRoot.range[1];
  if (tokensToEndOfLine.length > 0) ***REMOVED***
    endOfLine = tokensToEndOfLine[tokensToEndOfLine.length - 1].range[1];
  ***REMOVED***
  return fixer => fixer.insertTextAfterRange([prevRoot.range[0], endOfLine], '\n');
***REMOVED***

function removeNewLineAfterImport(context, currentImport, previousImport) ***REMOVED***
  const sourceCode = context.getSourceCode();
  const prevRoot = findRootNode(previousImport.node);
  ...
  if (/^\s*$/.test(sourceCode.text.substring(rangeToRemove[0], rangeToRemove[1]))) ***REMOVED***
    return fixer => fixer.removeRange(rangeToRemove);
  ***REMOVED***
  return undefined;
***REMOVED***

function makeNewlinesBetweenReport(context, imported, newlinesBetweenImports) ***REMOVED***
  const getNumberOfEmptyLinesBetween = (currentImport, previousImport) => ***REMOVED***
  ...
  ***REMOVED***;
  ...
  imported.slice(1).forEach(function (currentImport) ***REMOVED***
    const emptyLinesBetween = getNumberOfEmptyLinesBetween(currentImport, previousImport);

    if (newlinesBetweenImports === 'always' || newlinesBetweenImports === 'always-and-inside-groups') ***REMOVED***
      if (currentImport.rank !== previousImport.rank && emptyLinesBetween === 0) ***REMOVED***
        context.report(***REMOVED***
          node: previousImport.node,
          message: 'There should be at least one empty line between import groups',
          fix: fixNewLineAfterImport(context, previousImport, currentImport)
        ***REMOVED***);
      ***REMOVED*** else if (currentImport.rank === previousImport.rank && emptyLinesBetween > 0 && newlinesBetweenImports !== 'always-and-inside-groups') ***REMOVED***
        context.report(***REMOVED***
          node: previousImport.node,
        ...
        ***REMOVED***);
      ***REMOVED***
    ***REMOVED*** else if (emptyLinesBetween > 0) ***REMOVED***
      context.report(***REMOVED***
        node: previousImport.node,
...
module.exports = ***REMOVED***
  ...
  ***REMOVED***,
  ...
  create: function importOrderRule(context) ***REMOVED***
    ...
    ***REMOVED***
    ...
    function incrementLevel() ***REMOVED***
      level++;
    ***REMOVED***
    function decrementLevel() ***REMOVED***
      level--;
    ***REMOVED***

    return ***REMOVED***
      ImportDeclaration: function handleImports(node) ***REMOVED***
        ...
        ***REMOVED***
      ***REMOVED***,
      CallExpression: function handleRequires(node) ***REMOVED***
        if (level !== 0 || !(0, _staticRequire2.default)(node) || !isInVariableDeclarator(node.parent)) ***REMOVED***
          return;
        ***REMOVED***
        const name = node.arguments[0].value;
        registerNode(context, node, name, 'require', ranks, imported);
      ***REMOVED***,
      'Program:exit': function reportAndReset() ***REMOVED***
        makeOutOfOrderReport(context, imported);

        if (newlinesBetweenImports !== 'ignore') ***REMOVED***
