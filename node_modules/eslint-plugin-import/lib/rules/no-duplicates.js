...
function _toArray(arr) ***REMOVED*** return Array.isArray(arr) ? arr : Array.from(arr); ***REMOVED***

function checkImports(imported, context) ***REMOVED***
  for (const _ref of imported.entries()) ***REMOVED***
    var _ref2 = _slicedToArray(_ref, 2);

    const module = _ref2[0];
    const nodes = _ref2[1];

    if (nodes.length > 1) ***REMOVED***
      const message = `'$***REMOVED***module***REMOVED***' imported multiple times.`;
      ...
      ***REMOVED***);
      ...
      for (const node of rest) ***REMOVED***
        context.report(***REMOVED***
          node: node.source,
          message
        ***REMOVED***);
...
function getFix(first, rest, sourceCode) ***REMOVED***
  ...
  // check for.
  if (typeof sourceCode.getCommentsBefore !== 'function') ***REMOVED***
    return undefined;
  ***REMOVED***
  ...
  if (hasProblematicComments(first, sourceCode) || hasNamespace(first)) ***REMOVED***
    return undefined;
  ***REMOVED***
  ...
  if (defaultImportNames.size > 1) ***REMOVED***
    return undefined;
  ***REMOVED***
  ...
  const specifiers = restWithoutComments.map(node => ***REMOVED***
    const tokens = sourceCode.getTokens(node);
    const openBrace = tokens.find(token => isPunctuator(token, '***REMOVED***'));
    const closeBrace = tokens.find(token => isPunctuator(token, '***REMOVED***'));

    if (openBrace == null || closeBrace == null) ***REMOVED***
      return undefined;
    ***REMOVED***
  ...
  ***REMOVED***).filter(Boolean);
  ...
  if (!(shouldAddDefault || shouldAddSpecifiers || shouldRemoveUnnecessary)) ***REMOVED***
    return undefined;
  ***REMOVED***
  ...
  return fixer => ***REMOVED***
    ...
    var _specifiers$reduce = specifiers.reduce((_ref3, specifier) => ***REMOVED***
      var _ref4 = _slicedToArray(_ref3, 2);

      let result = _ref4[0],
          needsComma = _ref4[1];

      return [needsComma && !specifier.isEmpty ? `$***REMOVED***result***REMOVED***,$***REMOVED***specifier.text***REMOVED***` : `$***REMOVED***result***REMOVED***$***REMOVED***specifier.text***REMOVED***`, specifier.isEmpty ? needsComma : true];
    ***REMOVED***, ['', !firstHasTrailingComma && !firstIsEmpty]),
    ...
    if (shouldAddDefault && openBrace == null && shouldAddSpecifiers) ***REMOVED***
      // `import './foo'` → `import def, ***REMOVED***...***REMOVED*** from './foo'`
      fixes.push(fixer.insertTextAfter(firstToken, ` $***REMOVED***defaultImportName***REMOVED***, ***REMOVED***$***REMOVED***specifiersText***REMOVED******REMOVED*** from`));
    ***REMOVED*** else if (shouldAddDefault && openBrace == null && !shouldAddSpecifiers) ***REMOVED***
      // `import './foo'` → `import def from './foo'`
      fixes.push(fixer.insertTextAfter(firstToken, ` $***REMOVED***defaultImportName***REMOVED*** from`));
    ***REMOVED*** else if (shouldAddDefault && openBrace != null && closeBrace != null) ***REMOVED***
      // `import ***REMOVED***...***REMOVED*** from './foo'` → `import def, ***REMOVED***...***REMOVED*** from './foo'`
      fixes.push(fixer.insertTextAfter(firstToken, ` $***REMOVED***defaultImportName***REMOVED***,`));
      if (shouldAddSpecifiers) ***REMOVED***
        // `import def, ***REMOVED***...***REMOVED*** from './foo'` → `import def, ***REMOVED***..., ...***REMOVED*** from './foo'`
        fixes.push(fixer.insertTextBefore(closeBrace, specifiersText));
      ***REMOVED***
    ***REMOVED*** else if (!shouldAddDefault && openBrace == null && shouldAddSpecifiers) ***REMOVED***
      // `import './foo'` → `import ***REMOVED***...***REMOVED*** from './foo'`
      fixes.push(fixer.insertTextAfter(firstToken, ` ***REMOVED***$***REMOVED***specifiersText***REMOVED******REMOVED*** from`));
    ***REMOVED*** else if (!shouldAddDefault && openBrace != null && closeBrace != null) ***REMOVED***
      // `import ***REMOVED***...***REMOVED*** './foo'` → `import ***REMOVED***..., ...***REMOVED*** from './foo'`
      fixes.push(fixer.insertTextBefore(closeBrace, specifiersText));
    ***REMOVED***
    ...
    ***REMOVED***
    ...
    // import.
    for (const node of unnecessaryImports) ***REMOVED***
...
function hasProblematicComments(node, sourceCode) ***REMOVED***
...
function hasCommentBefore(node, sourceCode) ***REMOVED***
  return sourceCode.getCommentsBefore(node).some(comment => comment.loc.end.line >= node.loc.start.line - 1);
***REMOVED***

// Checks whether `node` has a comment (that starts) on the same line as `node`
// (ends).
function hasCommentAfter(node, sourceCode) ***REMOVED***
  return sourceCode.getCommentsAfter(node).some(comment => comment.loc.start.line === node.loc.end.line);
***REMOVED***

// Checks whether `node` has any comments _inside,_ except inside the `***REMOVED***...***REMOVED***`
// part (if any).
function hasCommentInsideNonSpecifiers(node, sourceCode) ***REMOVED***
  const tokens = sourceCode.getTokens(node);
  const openBraceIndex = tokens.findIndex(token => isPunctuator(token, '***REMOVED***'));
  const closeBraceIndex = tokens.findIndex(token => isPunctuator(token, '***REMOVED***'));
  // Slice away the first token, since we're no looking for comments _before_
  // `node` (only inside). If there's a `***REMOVED***...***REMOVED***` part, look for comments before
  // the `***REMOVED***`, but not before the `***REMOVED***` (hence the `+1`s).
...
module.exports = ***REMOVED***
  ...
  ***REMOVED***,
  ...
  create: function (context) ***REMOVED***
    ...
    return ***REMOVED***
      'ImportDeclaration': function (n) ***REMOVED***
        ...
        if (importMap.has(resolvedPath)) ***REMOVED***
