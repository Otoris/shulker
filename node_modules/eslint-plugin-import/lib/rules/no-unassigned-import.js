...
function report(context, node) ***REMOVED***
...
function testIsAllow(globs, filename, source) ***REMOVED***
  if (!Array.isArray(globs)) ***REMOVED***
    return false; // default doesn't allow any patterns
  ***REMOVED***

  let filePath;

  if (source[0] !== '.' && source[0] !== '/') ***REMOVED***
    // a node module
    filePath = source;
...
function create(context) ***REMOVED***
  ...
  return ***REMOVED***
    ImportDeclaration(node) ***REMOVED***
      if (node.specifiers.length === 0 && !isAllow(node.source.value)) ***REMOVED***
        report(context, node);
      ***REMOVED***
    ***REMOVED***,
    ExpressionStatement(node) ***REMOVED***
      if (node.expression.type === 'CallExpression' && (0, _staticRequire2.default)(node.expression) && !isAllow(node.expression.arguments[0].value)) ***REMOVED***
