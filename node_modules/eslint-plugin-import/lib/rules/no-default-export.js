'use strict';

module.exports = ***REMOVED***
  meta: ***REMOVED***
    type: 'suggestion',
    docs: ***REMOVED******REMOVED***
  ***REMOVED***,

  create(context) ***REMOVED***
    // ignore non-modules
    if (context.parserOptions.sourceType !== 'module') ***REMOVED***
      return ***REMOVED******REMOVED***;
    ***REMOVED***

    const preferNamed = 'Prefer named exports.';
    const noAliasDefault = (_ref) => ***REMOVED***
      let local = _ref.local;
      return `Do not alias \`$***REMOVED***local.name***REMOVED***\` as \`default\`. Just export ` + `\`$***REMOVED***local.name***REMOVED***\` itself instead.`;
    ***REMOVED***;

    return ***REMOVED***
      ExportDefaultDeclaration(node) ***REMOVED***
        context.report(***REMOVED*** node, message: preferNamed ***REMOVED***);
      ***REMOVED***,

      ExportNamedDeclaration(node) ***REMOVED***
        node.specifiers.forEach(specifier => ***REMOVED***
          if (specifier.type === 'ExportDefaultSpecifier' && specifier.exported.name === 'default') ***REMOVED***
            context.report(***REMOVED*** node, message: preferNamed ***REMOVED***);
          ***REMOVED*** else if (specifier.type === 'ExportSpecifier' && specifier.exported.name === 'default') ***REMOVED***
            context.report(***REMOVED*** node, message: noAliasDefault(specifier) ***REMOVED***);
          ***REMOVED***
        ***REMOVED***);
      ***REMOVED***
    ***REMOVED***;
  ***REMOVED***
***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,