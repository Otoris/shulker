...
module.exports = ***REMOVED***
  ...
  ***REMOVED***,
  ...
  create: function noReachingInside(context) ***REMOVED***
    ...
    function reachingAllowed(importPath) ***REMOVED***
      return allowRegexps.some(re => re.test(importPath));
    ***REMOVED***

    // minimatch patterns are expected to use / path separators, like import
    // statements, so normalize paths to use the same
    function normalizeSep(somePath) ***REMOVED***
      return somePath.split('\\').join('/');
    ***REMOVED***

    // find a directory that is being reached into, but which shouldn't be
    function isReachViolation(importPath) ***REMOVED***
      const steps = normalizeSep(importPath).split('/').reduce((acc, step) => ***REMOVED***
        if (!step || step === '.') ***REMOVED***
          return acc;
        ***REMOVED*** else if (step === '..') ***REMOVED***
      ...
      ***REMOVED***, []);
      ...
      if (reachingAllowed(justSteps) || reachingAllowed(`/$***REMOVED***justSteps***REMOVED***`)) return false;
    ...
    ***REMOVED***
    ...
    function checkImportForReaching(importPath, node) ***REMOVED***
      const potentialViolationTypes = ['parent', 'index', 'sibling', 'external', 'internal'];
      if (potentialViolationTypes.indexOf((0, _importType2.default)(importPath, context)) !== -1 && isReachViolation(importPath)) ***REMOVED***
        context.report(***REMOVED***
          node,
          message: `Reaching to "$***REMOVED***importPath***REMOVED***" is not allowed.`
        ***REMOVED***);
      ***REMOVED***
    ***REMOVED***
