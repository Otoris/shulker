...
module.exports = ***REMOVED***
  ...
  ***REMOVED***,
  ...
  create: function (context) ***REMOVED***
    function checkSpecifiers(key, type, node) ***REMOVED***
      // ignore local exports and type imports/exports
      if (node.source == null || node.importKind === 'type' || node.importKind === 'typeof' || node.exportKind === 'type') ***REMOVED***
        return;
      ***REMOVED***

      if (!node.specifiers.some(function (im) ***REMOVED***
        return im.type === type;
      ***REMOVED***)) ***REMOVED***
      ...
      ***REMOVED***
      ...
      node.specifiers.forEach(function (im) ***REMOVED***
        ...
        if (!deepLookup.found) ***REMOVED***
          if (deepLookup.path.length > 1) ***REMOVED***
            const deepPath = deepLookup.path.map(i => path.relative(path.dirname(context.getFilename()), i.path)).join(' -> ');

            context.report(im[key], `$***REMOVED***im[key].name***REMOVED*** not found via $***REMOVED***deepPath***REMOVED***`);
