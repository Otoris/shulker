'use strict';

var _docsUrl = require('../docsUrl');

var _docsUrl2 = _interopRequireDefault(_docsUrl);

function _interopRequireDefault(obj) ***REMOVED*** return obj && obj.__esModule ? obj : ***REMOVED*** default: obj ***REMOVED***; ***REMOVED***

module.exports = ***REMOVED***
  meta: ***REMOVED***
    type: 'suggestion',
    docs: ***REMOVED***
      url: (0, _docsUrl2.default)('no-mutable-exports')
    ***REMOVED***
  ***REMOVED***,

  create: function (context) ***REMOVED***
    function checkDeclaration(node) ***REMOVED***
      const kind = node.kind;

      if (kind === 'var' || kind === 'let') ***REMOVED***
        context.report(node, `Exporting mutable '$***REMOVED***kind***REMOVED***' binding, use 'const' instead.`);
      ***REMOVED***
    ***REMOVED***

    function checkDeclarationsInScope(_ref, name) ***REMOVED***
      let variables = _ref.variables;

      for (let variable of variables) ***REMOVED***
        if (variable.name === name) ***REMOVED***
          for (let def of variable.defs) ***REMOVED***
            if (def.type === 'Variable' && def.parent) ***REMOVED***
              checkDeclaration(def.parent);
            ***REMOVED***
          ***REMOVED***
        ***REMOVED***
      ***REMOVED***
    ***REMOVED***

    function handleExportDefault(node) ***REMOVED***
      const scope = context.getScope();

      if (node.declaration.name) ***REMOVED***
        checkDeclarationsInScope(scope, node.declaration.name);
      ***REMOVED***
    ***REMOVED***

    function handleExportNamed(node) ***REMOVED***
      const scope = context.getScope();

      if (node.declaration) ***REMOVED***
        checkDeclaration(node.declaration);
      ***REMOVED*** else if (!node.source) ***REMOVED***
        for (let specifier of node.specifiers) ***REMOVED***
          checkDeclarationsInScope(scope, specifier.local.name);
        ***REMOVED***
      ***REMOVED***
    ***REMOVED***

    return ***REMOVED***
      'ExportDefaultDeclaration': handleExportDefault,
      'ExportNamedDeclaration': handleExportNamed
    ***REMOVED***;
  ***REMOVED***
***REMOVED***;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,