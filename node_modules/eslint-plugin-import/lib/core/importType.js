...
function baseModule(name) ***REMOVED***
  if (isScoped(name)) ***REMOVED***
    ...
    return `$***REMOVED***scope***REMOVED***/$***REMOVED***pkg***REMOVED***`;
  ***REMOVED***
...
function isAbsolute(name) ***REMOVED***
  return name.indexOf('/') === 0;
***REMOVED***

// path is defined only when a resolver resolves to a non-standard path
function isBuiltIn(name, settings, path) ***REMOVED***
...
function isExternalPath(path, name, settings) ***REMOVED***
...
function isExternalModule(name, settings, path) ***REMOVED***
  return externalModuleRegExp.test(name) && isExternalPath(path, name, settings);
***REMOVED***

const externalModuleMainRegExp = /^[\w]((?!\/).)*$/;
function isExternalModuleMain(name, settings, path) ***REMOVED***
  return externalModuleMainRegExp.test(name) && isExternalPath(path, name, settings);
***REMOVED***

const scopedRegExp = /^@[^/]+\/[^/]+/;
function isScoped(name) ***REMOVED***
  return scopedRegExp.test(name);
***REMOVED***

const scopedMainRegExp = /^@[^/]+\/?[^/]+$/;
function isScopedMain(name) ***REMOVED***
  return scopedMainRegExp.test(name);
***REMOVED***

function isInternalModule(name, settings, path) ***REMOVED***
...
function isRelativeToParent(name) ***REMOVED***
  return (/^\.\.[\\/]/.test(name)
  );
***REMOVED***

const indexFiles = ['.', './', './index', './index.js'];
function isIndex(name) ***REMOVED***
  return indexFiles.indexOf(name) !== -1;
***REMOVED***

function isRelativeToSibling(name) ***REMOVED***
  return (/^\.[\\/]/.test(name)
  );
***REMOVED***

function typeTest(name, settings, path) ***REMOVED***
  if (isAbsolute(name, settings, path)) ***REMOVED***
    return 'absolute';
  ***REMOVED***
  if (isBuiltIn(name, settings, path)) ***REMOVED***
    return 'builtin';
  ***REMOVED***
  if (isInternalModule(name, settings, path)) ***REMOVED***
    return 'internal';
  ***REMOVED***
  if (isExternalModule(name, settings, path)) ***REMOVED***
    return 'external';
  ***REMOVED***
  if (isScoped(name, settings, path)) ***REMOVED***
    return 'external';
  ***REMOVED***
  if (isRelativeToParent(name, settings, path)) ***REMOVED***
    return 'parent';
  ***REMOVED***
  if (isIndex(name, settings, path)) ***REMOVED***
    return 'index';
  ***REMOVED***
  if (isRelativeToSibling(name, settings, path)) ***REMOVED***
    return 'sibling';
  ***REMOVED***
  return 'unknown';
***REMOVED***

function resolveImportType(name, context) ***REMOVED***
