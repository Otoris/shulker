...
* parseOptions - `***REMOVED***slice: Int***REMOVED***` - all options optional
...
##### example
```js
parse(['node', 't.js', '--count', '2', 'positional']); // ***REMOVED***count: 2, _: ['positional']***REMOVED***
parse('--count 2 positional');                         // ***REMOVED***count: 2, _: ['positional']***REMOVED***
parse(***REMOVED***count: 2, _:['positional']***REMOVED***);                   // ***REMOVED***count: 2, _: ['positional']***REMOVED***
```
...
##### arguments
* helpOptions - `***REMOVED***showHidden: Boolean, interpolate: Object***REMOVED***` - all options optional
    - `showHidden` specifies whether to show options with `hidden: true` specified, by default it is `false`
    - `interpolate` specify data to be interpolated in `prepend` and `append` text, `***REMOVED******REMOVED***key***REMOVED******REMOVED***` is the format - eg. `generateHelp(***REMOVED***interpolate:***REMOVED***version: '0.4.2'***REMOVED******REMOVED***)`, will change this `append` text: `Version ***REMOVED******REMOVED***version***REMOVED******REMOVED***` to `Version 0.4.2`
...
When your `require('optionator')`, you get a function that takes in a settings object. This object has the type:
    ...
    ***REMOVED***
      prepend: String,
      append: String,
      options: [***REMOVED***heading: String***REMOVED*** | ***REMOVED***
        option: String,
        alias: [String] | String,
        type: String,
        enum: [String],
...
* `typeAliases` is an optional object, it allows you to set aliases for types, eg. `***REMOVED***Path: 'String'***REMOVED***` would allow you to use the type `Path` as an alias for the type `String`
* `defaults` is an optional object following the option properties format, which specifies default values for all options. A default will be overridden if manually set. For example, you can do `default: ***REMOVED*** type: "String" ***REMOVED***` to set the default type of all options to `String`, and then override that default in an individual option by setting the `type` property
...
* `mergeRepeatedObjects` is an optional boolean (defaults to `false`) - when set to `true` and an option contains an object value and is repeated, the subsequent values for the flag will be merged rather than overwriting the original value - eg. option `g` of type `Object`: `-g a:1 -g b:2 -g c:3,d:4` will result in `***REMOVED***a: 1, b: 2, c: 3, d: 4***REMOVED***`
