"use strict";
Object.defineProperty(exports, "__esModule", ***REMOVED*** value: true ***REMOVED***);
var tslib_1 = require("tslib");
var ts = require("typescript");
var Lint = require("tslint");
var Rule = (function (_super) ***REMOVED***
    tslib_1.__extends(Rule, _super);
    function Rule() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
    ***REMOVED***
    Rule.prototype.apply = function (sourceFile) ***REMOVED***
        var walker = new ValidTypeofWalker(sourceFile, this.getOptions());
        return this.applyWithWalker(walker);
    ***REMOVED***;
    Rule.FAILURE_STRING = 'invalid typeof comparison value';
    return Rule;
***REMOVED***(Lint.Rules.AbstractRule));
exports.Rule = Rule;
var ValidTypeofWalker = (function (_super) ***REMOVED***
    tslib_1.__extends(ValidTypeofWalker, _super);
    function ValidTypeofWalker() ***REMOVED***
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.VALID_TYPES = ['symbol', 'undefined', 'object', 'boolean', 'number', 'string', 'function'];
        _this.OPERATORS = [ts.SyntaxKind.EqualsEqualsToken, ts.SyntaxKind.EqualsEqualsEqualsToken, ts.SyntaxKind.ExclamationEqualsToken, ts.SyntaxKind.ExclamationEqualsEqualsToken];
        return _this;
    ***REMOVED***
    ValidTypeofWalker.prototype.visitNode = function (node) ***REMOVED***
        if (node.kind === ts.SyntaxKind.TypeOfExpression) ***REMOVED***
            this.validateTypeOf(node);
        ***REMOVED***
        _super.prototype.visitNode.call(this, node);
    ***REMOVED***;
    ValidTypeofWalker.prototype.validateTypeOf = function (node) ***REMOVED***
        if (node.parent && node.parent.kind === ts.SyntaxKind.BinaryExpression) ***REMOVED***
            var parent = node.parent;
            if (this.OPERATORS.indexOf(parent.operatorToken.kind) !== -1) ***REMOVED***
                var sibling = parent.left === node ? parent.right : parent.left;
                if (sibling.kind === ts.SyntaxKind.StringLiteral && this.VALID_TYPES.indexOf(sibling.text) === -1) ***REMOVED***
                    this.addFailure(this.createFailure(node.getStart(), node.getWidth(), Rule.FAILURE_STRING));
                ***REMOVED***
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***;
    return ValidTypeofWalker;
***REMOVED***(Lint.RuleWalker));

//# sourceMappingURL=data:application/json;charset=utf8;base64,
