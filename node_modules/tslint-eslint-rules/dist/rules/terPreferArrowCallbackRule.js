"use strict";
Object.defineProperty(exports, "__esModule", ***REMOVED*** value: true ***REMOVED***);
var tslib_1 = require("tslib");
var ts = require("typescript");
var Lint = require("tslint");
var RULE_NAME = 'ter-prefer-arrow-callback';
var OPTIONS;
var Rule = (function (_super) ***REMOVED***
    tslib_1.__extends(Rule, _super);
    function Rule() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
    ***REMOVED***
    Rule.prototype.apply = function (sourceFile) ***REMOVED***
        var walker = new RuleWalker(sourceFile, this.getOptions());
        return this.applyWithWalker(walker);
    ***REMOVED***;
    Rule.metadata = ***REMOVED***
        ruleName: RULE_NAME,
        description: 'require arrow functions as callbacks',
        rationale: Lint.Utils.dedent(templateObject_1 || (templateObject_1 = tslib_1.__makeTemplateObject(["\n      Arrow functions are suited to callbacks, because:\n\n      * `this` keywords in arrow functions bind to the upper scope\u2019s.\n      * The notation of the arrow function is shorter than function expression\u2019s.\n      "], ["\n      Arrow functions are suited to callbacks, because:\n\n      * \\`this\\` keywords in arrow functions bind to the upper scope\u2019s.\n      * The notation of the arrow function is shorter than function expression\u2019s.\n      "]))),
        optionsDescription: Lint.Utils.dedent(templateObject_2 || (templateObject_2 = tslib_1.__makeTemplateObject(["\n      This rule takes one optional argument, an object which is an options object. This object\n      may specify the following properties:\n\n      * `\"allowNamedFunctions\"` (default false) When set to `true`, the rule doesn't warn on\n                                  named functions used as callback.\n      * `\"allowUnboundThis\"` (default true) When set to `false`, this option allows the use of\n                               `this` without restriction and checks for dynamically assigned\n                               `this` values such as when using `Array.prototype.map` with a\n                               `context` argument. Normally, the rule will flag the use of this\n                               whenever a function does not use `bind()` to specify the value of\n                               `this` constantly.\n      "], ["\n      This rule takes one optional argument, an object which is an options object. This object\n      may specify the following properties:\n\n      * \\`\"allowNamedFunctions\"\\` (default false) When set to \\`true\\`, the rule doesn't warn on\n                                  named functions used as callback.\n      * \\`\"allowUnboundThis\"\\` (default true) When set to \\`false\\`, this option allows the use of\n                               \\`this\\` without restriction and checks for dynamically assigned\n                               \\`this\\` values such as when using \\`Array.prototype.map\\` with a\n                               \\`context\\` argument. Normally, the rule will flag the use of this\n                               whenever a function does not use \\`bind()\\` to specify the value of\n                               \\`this\\` constantly.\n      "]))),
        options: ***REMOVED***
            type: 'array',
            items: [***REMOVED***
                    type: 'object',
                    properties: ***REMOVED***
                        allowNamedFunctions: ***REMOVED***
                            type: 'boolean'
                        ***REMOVED***,
                        allowUnboundThis: ***REMOVED***
                            type: 'boolean'
                        ***REMOVED***
                    ***REMOVED***,
                    additionalProperties: false
                ***REMOVED***],
            maxLength: 1
        ***REMOVED***,
        optionExamples: [
            Lint.Utils.dedent(templateObject_3 || (templateObject_3 = tslib_1.__makeTemplateObject(["\n        \"", "\": [true]\n        "], ["\n        \"", "\": [true]\n        "])), RULE_NAME),
            Lint.Utils.dedent(templateObject_4 || (templateObject_4 = tslib_1.__makeTemplateObject(["\n        \"", "\": [true, ***REMOVED***\n          \"allowNamedFunctions\": true\n        ***REMOVED***]\n        "], ["\n        \"", "\": [true, ***REMOVED***\n          \"allowNamedFunctions\": true\n        ***REMOVED***]\n        "])), RULE_NAME),
            Lint.Utils.dedent(templateObject_5 || (templateObject_5 = tslib_1.__makeTemplateObject(["\n        \"", "\": [true, ***REMOVED***\n          \"allowUnboundThis\": false\n        ***REMOVED***]\n        "], ["\n        \"", "\": [true, ***REMOVED***\n          \"allowUnboundThis\": false\n        ***REMOVED***]\n        "])), RULE_NAME),
            Lint.Utils.dedent(templateObject_6 || (templateObject_6 = tslib_1.__makeTemplateObject(["\n        \"", "\": [true, ***REMOVED***\n          \"allowNamedFunctions\": true,\n          \"allowUnboundThis\": false\n        ***REMOVED***]\n        "], ["\n        \"", "\": [true, ***REMOVED***\n          \"allowNamedFunctions\": true,\n          \"allowUnboundThis\": false\n        ***REMOVED***]\n        "])), RULE_NAME)
        ],
        typescriptOnly: false,
        type: 'typescript'
    ***REMOVED***;
    return Rule;
***REMOVED***(Lint.Rules.AbstractRule));
exports.Rule = Rule;
function checkMetaProperty(node, name, prop) ***REMOVED***
    return node.parent && node.parent.getFirstToken().getText() === name && node.name.text === prop;
***REMOVED***
function getCallbackInfo(func) ***REMOVED***
    var retv = ***REMOVED*** isCallback: false, isLexicalThis: false ***REMOVED***;
    var node = func;
    var parent = node.parent;
    while (node && parent) ***REMOVED***
        switch (parent.kind) ***REMOVED***
            case ts.SyntaxKind.BinaryExpression:
            case ts.SyntaxKind.ConditionalExpression:
                break;
            case ts.SyntaxKind.PropertyAccessExpression:
                if (parent.name.kind === ts.SyntaxKind.Identifier &&
                    parent.name.text === 'bind' &&
                    parent.parent &&
                    parent.parent.kind === ts.SyntaxKind.CallExpression &&
                    parent.parent.expression === parent) ***REMOVED***
                    retv.isLexicalThis = (parent.parent.arguments.length === 1 &&
                        parent.parent.arguments[0].kind === ts.SyntaxKind.ThisKeyword);
                    node = parent;
                    parent = parent.parent;
                ***REMOVED***
                else ***REMOVED***
                    return retv;
                ***REMOVED***
                break;
            case ts.SyntaxKind.CallExpression:
            case ts.SyntaxKind.NewExpression:
                if (parent.expression !== node) ***REMOVED***
                    retv.isCallback = true;
                ***REMOVED***
                return retv;
            default:
                return retv;
        ***REMOVED***
        node = parent;
        parent = node.parent;
    ***REMOVED***
    throw new Error('unreachable');
***REMOVED***
var RuleWalker = (function (_super) ***REMOVED***
    tslib_1.__extends(RuleWalker, _super);
    function RuleWalker(sourceFile, options) ***REMOVED***
        var _this = _super.call(this, sourceFile, options) || this;
        _this.stack = [];
        OPTIONS = ***REMOVED***
            allowUnboundThis: true,
            allowNamedFunctions: null
        ***REMOVED***;
        var userOptions = _this.getOptions()[0];
        if (userOptions) ***REMOVED***
            OPTIONS.allowUnboundThis = userOptions.allowUnboundThis !== false;
            OPTIONS.allowNamedFunctions = userOptions.allowNamedFunctions;
        ***REMOVED***
        return _this;
    ***REMOVED***
    RuleWalker.prototype.enterScope = function (functionName) ***REMOVED***
        this.stack.push(***REMOVED***
            functionName: functionName,
            isRecursive: false,
            hasThis: false,
            hasSuper: false,
            hasMeta: false,
            hasArguments: false
        ***REMOVED***);
    ***REMOVED***;
    RuleWalker.prototype.exitScope = function () ***REMOVED***
        return this.stack.pop();
    ***REMOVED***;
    RuleWalker.prototype.exitFunctionExpression = function (node) ***REMOVED***
        var scopeInfo = this.exitScope();
        if (node.asteriskToken) ***REMOVED***
            return;
        ***REMOVED***
        if (node.name && node.name.text) ***REMOVED***
            if (OPTIONS.allowNamedFunctions || scopeInfo.isRecursive) ***REMOVED***
                return;
            ***REMOVED***
        ***REMOVED***
        var params = node.parameters.map(function (x) ***REMOVED*** return x.name.getText(); ***REMOVED***);
        var argumentsIsParam = params.indexOf('arguments') !== -1;
        if (!argumentsIsParam && scopeInfo.hasArguments) ***REMOVED***
            return;
        ***REMOVED***
        var callbackInfo = getCallbackInfo(node);
        if (callbackInfo.isCallback &&
            (!OPTIONS.allowUnboundThis || !scopeInfo.hasThis || callbackInfo.isLexicalThis) &&
            !scopeInfo.hasSuper &&
            !scopeInfo.hasMeta) ***REMOVED***
            var failure = this.createFailure(node.getStart(), node.getWidth(), 'Unexpected function expression.');
            this.addFailure(failure);
        ***REMOVED***
    ***REMOVED***;
    RuleWalker.prototype.visitSourceFile = function (node) ***REMOVED***
        this.stack = [];
        _super.prototype.visitSourceFile.call(this, node);
    ***REMOVED***;
    RuleWalker.prototype.visitFunctionDeclaration = function (node) ***REMOVED***
        this.enterScope();
        _super.prototype.visitFunctionDeclaration.call(this, node);
        this.exitScope();
    ***REMOVED***;
    RuleWalker.prototype.visitFunctionExpression = function (node) ***REMOVED***
        this.enterScope(node.name ? node.name.text : undefined);
        _super.prototype.visitFunctionExpression.call(this, node);
        this.exitFunctionExpression(node);
    ***REMOVED***;
    RuleWalker.prototype.visitNode = function (node) ***REMOVED***
        var info = this.stack[this.stack.length - 1];
        if (info && node.parent && node.parent.kind !== ts.SyntaxKind.FunctionExpression) ***REMOVED***
            if (node.kind === ts.SyntaxKind.ThisKeyword) ***REMOVED***
                info.hasThis = true;
            ***REMOVED***
            else if (node.kind === ts.SyntaxKind.SuperKeyword) ***REMOVED***
                info.hasSuper = true;
            ***REMOVED***
            else if (node.kind === ts.SyntaxKind.Identifier) ***REMOVED***
                var text = node.text;
                if (text === 'arguments') ***REMOVED***
                    info.hasArguments = true;
                ***REMOVED***
                else if (text === info.functionName) ***REMOVED***
                    info.isRecursive = true;
                ***REMOVED***
            ***REMOVED***
            else if ((node.kind === ts.SyntaxKind.PropertyAccessExpression ||
                node.kind === ts.SyntaxKind.MetaProperty) &&
                checkMetaProperty(node, 'new', 'target')) ***REMOVED***
                info.hasMeta = true;
            ***REMOVED***
        ***REMOVED***
        _super.prototype.visitNode.call(this, node);
    ***REMOVED***;
    return RuleWalker;
***REMOVED***(Lint.RuleWalker));
var templateObject_1, templateObject_2, templateObject_3, templateObject_4, templateObject_5, templateObject_6;

//# sourceMappingURL=data:application/json;charset=utf8;base64,
