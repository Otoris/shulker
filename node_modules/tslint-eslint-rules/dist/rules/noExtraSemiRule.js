"use strict";
Object.defineProperty(exports, "__esModule", ***REMOVED*** value: true ***REMOVED***);
var tslib_1 = require("tslib");
var ts = require("typescript");
var Lint = require("tslint");
var Rule = (function (_super) ***REMOVED***
    tslib_1.__extends(Rule, _super);
    function Rule() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
    ***REMOVED***
    Rule.prototype.apply = function (sourceFile) ***REMOVED***
        var walker = new NoExtraSemiWalker(sourceFile, this.getOptions());
        return this.applyWithWalker(walker);
    ***REMOVED***;
    Rule.FAILURE_STRING = 'unnecessary semicolon';
    return Rule;
***REMOVED***(Lint.Rules.AbstractRule));
exports.Rule = Rule;
var NoExtraSemiWalker = (function (_super) ***REMOVED***
    tslib_1.__extends(NoExtraSemiWalker, _super);
    function NoExtraSemiWalker() ***REMOVED***
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.ALLOWED_PARENT_TYPES = [
            ts.SyntaxKind.ForStatement,
            ts.SyntaxKind.ForInStatement,
            ts.SyntaxKind.ForOfStatement,
            ts.SyntaxKind.WhileStatement,
            ts.SyntaxKind.DoStatement
        ];
        return _this;
    ***REMOVED***
    NoExtraSemiWalker.prototype.visitNode = function (node) ***REMOVED***
        if (node.kind === ts.SyntaxKind.EmptyStatement) ***REMOVED***
            this.visitEmptyStatement(node);
        ***REMOVED***
        _super.prototype.visitNode.call(this, node);
    ***REMOVED***;
    NoExtraSemiWalker.prototype.visitClassDeclaration = function (node) ***REMOVED***
        this.checkClass(node);
        _super.prototype.visitClassDeclaration.call(this, node);
    ***REMOVED***;
    NoExtraSemiWalker.prototype.visitEmptyStatement = function (node) ***REMOVED***
        if (node.parent && this.ALLOWED_PARENT_TYPES.indexOf(node.parent.kind) === -1) ***REMOVED***
            this.validateNoExtraSemi(node);
        ***REMOVED***
    ***REMOVED***;
    NoExtraSemiWalker.prototype.checkClass = function (node) ***REMOVED***
        var indexOf = node.getChildren().map(function (child) ***REMOVED*** return child.kind; ***REMOVED***).indexOf(ts.SyntaxKind.FirstPunctuation);
        var children = node.getChildren().slice(indexOf);
        this.checkClassChildren(children);
    ***REMOVED***;
    NoExtraSemiWalker.prototype.checkClassChildren = function (children) ***REMOVED***
        for (var _i = 0, children_1 = children; _i < children_1.length; _i++) ***REMOVED***
            var child = children_1[_i];
            if ((child.kind === ts.SyntaxKind.SyntaxList || child.kind === ts.SyntaxKind.SemicolonClassElement) && child.getText() === ';') ***REMOVED***
                this.validateNoExtraSemi(child);
            ***REMOVED***
            else if (child.kind === ts.SyntaxKind.SyntaxList && child.getText().indexOf(';') !== -1) ***REMOVED***
                this.checkClassChildren(child.getChildren());
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***;
    NoExtraSemiWalker.prototype.validateNoExtraSemi = function (node) ***REMOVED***
        this.addFailure(this.createFailure(node.getStart(), node.getWidth(), Rule.FAILURE_STRING));
    ***REMOVED***;
    return NoExtraSemiWalker;
***REMOVED***(Lint.RuleWalker));

//# sourceMappingURL=data:application/json;charset=utf8;base64,
