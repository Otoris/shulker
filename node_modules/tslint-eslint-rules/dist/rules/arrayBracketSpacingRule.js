                ...
                ***REMOVED***
                    ...
                    properties: ***REMOVED***
                        singleValue: ***REMOVED***
                            type: 'boolean'
                        ***REMOVED***,
                        objectsInArrays: ***REMOVED***
                            type: 'boolean'
                        ***REMOVED***,
                        arraysInArrays: ***REMOVED***
                            type: 'boolean'
                        ***REMOVED***
                    ***REMOVED***,
...
exports.Rule = Rule;
var ArrayBracketSpacingWalker = (function (_super) ***REMOVED***
    tslib_1.__extends(ArrayBracketSpacingWalker, _super);
    function ArrayBracketSpacingWalker(sourceFile, options) ***REMOVED***
        ...
        var isDef = function (x) ***REMOVED*** return typeof x !== 'undefined'; ***REMOVED***;
        if (opt) ***REMOVED***
        ...
        ***REMOVED***
        return _this;
    ***REMOVED***
    ArrayBracketSpacingWalker.prototype.report = function (start, msg, fix) ***REMOVED***
        this.addFailure(this.createFailure(start, 1, msg, fix));
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.reportNoBeginningSpace = function (token, space) ***REMOVED***
    ...
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.reportRequiredBeginningSpace = function (token) ***REMOVED***
        var start = token.getStart(this.getSourceFile());
        var fix = Lint.Replacement.appendText(start + 1, ' ');
        this.report(start, 'A space is required after "["', fix);
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.reportRequiredEndingSpace = function (token) ***REMOVED***
        var start = token.getStart(this.getSourceFile());
        var fix = Lint.Replacement.appendText(start, ' ');
        this.report(start, 'A space is required before "]"', fix);
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.reportNoEndingSpace = function (token, space) ***REMOVED***
        var start = token.getStart(this.getSourceFile());
        var fix = Lint.Replacement.deleteText(start - space, space);
        this.report(start, 'There should be no space before "]"', fix);
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.visitNode = function (node) ***REMOVED***
        if (node.kind === ts.SyntaxKind.ArrayBindingPattern) ***REMOVED***
            this.validateArraySpacing(node, node.elements);
        ***REMOVED***
        _super.prototype.visitNode.call(this, node);
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.visitArrayLiteralExpression = function (node) ***REMOVED***
        this.validateArraySpacing(node, node.elements);
        _super.prototype.visitArrayLiteralExpression.call(this, node);
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.isObjectType = function (node) ***REMOVED***
        return node && node.kind === ts.SyntaxKind.ObjectLiteralExpression;
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.isArrayType = function (node) ***REMOVED***
        if (node) ***REMOVED***
            if (node.kind === ts.SyntaxKind.ArrayLiteralExpression) ***REMOVED***
                return true;
            ***REMOVED***
            var firstChild = node.getChildAt(0);
            if (firstChild && firstChild.kind === ts.SyntaxKind.ArrayBindingPattern) ***REMOVED***
                return true;
            ***REMOVED***
        ***REMOVED***
        return false;
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.validateArraySpacing = function (node, elements) ***REMOVED***
        var _this = this;
        if (this.spaced && elements.length === 0) ***REMOVED***
            return;
        ***REMOVED***
        ...
        if (second.pos === second.end) ***REMOVED***
            second = node.getChildAt(1).getChildAt(1);
        ***REMOVED***
        if (elements.hasTrailingComma) ***REMOVED***
            penultimate = elements;
        ***REMOVED***
        var mustBeSpaced = function (token) ***REMOVED*** return (_this.singleValueException && elements.length === 1 ||
            _this.objectsInArraysException && _this.isObjectType(token) ||
            _this.arraysInArraysException && _this.isArrayType(token)) ? !_this.spaced : _this.spaced; ***REMOVED***;
        ...
        if (!isBreakAfterOpeningBracket) ***REMOVED***
            if (openingBracketMustBeSpaced && !spaceAfterOpeningBracket) ***REMOVED***
                this.reportRequiredBeginningSpace(first);
            ***REMOVED***
            else if (!openingBracketMustBeSpaced && spaceAfterOpeningBracket) ***REMOVED***
                this.reportNoBeginningSpace(first, spaceAfterOpeningBracket);
            ***REMOVED***
        ***REMOVED***
        if (first !== penultimate && !isBreakBeforeClosingBracket) ***REMOVED***
            if (closingBracketMustBeSpaced && !spaceBeforeClosingBracket) ***REMOVED***
                this.reportRequiredEndingSpace(last);
            ***REMOVED***
            else if (!closingBracketMustBeSpaced && spaceBeforeClosingBracket) ***REMOVED***
                this.reportNoEndingSpace(last, spaceBeforeClosingBracket);
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.getSpaceBetween = function (node, nextNode, trailing) ***REMOVED***
        var end = nextNode.getStart(this.getSourceFile());
        var start = node.end;
        var text = this.getSourceFile().text.substring(start, end);
        var m = text.match(/\/\*.*\*\//);
        if (m && typeof m.index === 'number') ***REMOVED***
            var len = m[0].length;
            return trailing ? end - (start + m.index + len) : m.index;
        ***REMOVED***
        return end - start;
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.isLineBreakBetween = function (node, nextNode) ***REMOVED***
        return this.getEndPosition(node).line !== this.getStartPosition(nextNode).line;
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.getStartPosition = function (node) ***REMOVED***
        var srcFile = this.getSourceFile();
        return srcFile.getLineAndCharacterOfPosition(node.getStart(srcFile));
    ***REMOVED***;
    ArrayBracketSpacingWalker.prototype.getEndPosition = function (node) ***REMOVED***
