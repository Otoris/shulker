"use strict";
Object.defineProperty(exports, "__esModule", ***REMOVED*** value: true ***REMOVED***);
var tslib_1 = require("tslib");
var ts = require("typescript");
var Lint = require("tslint");
var OPTION_ALWAYS = 'always';
var Rule = (function (_super) ***REMOVED***
    tslib_1.__extends(Rule, _super);
    function Rule() ***REMOVED***
        return _super !== null && _super.apply(this, arguments) || this;
    ***REMOVED***
    Rule.prototype.apply = function (sourceFile) ***REMOVED***
        var walker = new BlockSpacingWalker(sourceFile, this.getOptions());
        return this.applyWithWalker(walker);
    ***REMOVED***;
    Rule.FAILURE_STRING = ***REMOVED***
        always: 'Requires a space',
        never: 'Unexpected space(s)'
    ***REMOVED***;
    return Rule;
***REMOVED***(Lint.Rules.AbstractRule));
exports.Rule = Rule;
var BlockSpacingWalker = (function (_super) ***REMOVED***
    tslib_1.__extends(BlockSpacingWalker, _super);
    function BlockSpacingWalker(sourceFile, options) ***REMOVED***
        var _this = _super.call(this, sourceFile, options) || this;
        _this.always = _this.hasOption(OPTION_ALWAYS) || (_this.getOptions() && _this.getOptions().length === 0);
        return _this;
    ***REMOVED***
    BlockSpacingWalker.prototype.visitNode = function (node) ***REMOVED***
        if (node.kind === ts.SyntaxKind.Block || node.kind === ts.SyntaxKind.CaseBlock) ***REMOVED***
            this.checkSpacingInsideBraces(node);
        ***REMOVED***
        _super.prototype.visitNode.call(this, node);
    ***REMOVED***;
    BlockSpacingWalker.prototype.checkSpacingInsideBraces = function (node) ***REMOVED***
        var blockChildren = node.getChildren();
        var syntaxList = blockChildren[1];
        var openBraceLocation = this.getStartPosition(blockChildren[0]);
        var closeBraceLocation = this.getStartPosition(blockChildren[blockChildren.length - 1]);
        if (syntaxList && syntaxList.getChildCount() > 0 && openBraceLocation.line === closeBraceLocation.line) ***REMOVED***
            if (this.isSpaceBetween(blockChildren[0], blockChildren[1]) !== this.always
                || this.isSpaceBetween(blockChildren[blockChildren.length - 2], blockChildren[blockChildren.length - 1]) !== this.always) ***REMOVED***
                var failureString = this.always ? Rule.FAILURE_STRING.always : Rule.FAILURE_STRING.never;
                this.addFailure(this.createFailure(node.getStart(), node.getWidth(), failureString));
            ***REMOVED***
        ***REMOVED***
    ***REMOVED***;
    BlockSpacingWalker.prototype.isSpaceBetween = function (node, nextNode) ***REMOVED***
        return nextNode.getStart() - node.getEnd() > 0;
    ***REMOVED***;
    BlockSpacingWalker.prototype.getStartPosition = function (node) ***REMOVED***
        return node.getSourceFile().getLineAndCharacterOfPosition(node.getStart());
    ***REMOVED***;
    return BlockSpacingWalker;
***REMOVED***(Lint.RuleWalker));

//# sourceMappingURL=data:application/json;charset=utf8;base64,
