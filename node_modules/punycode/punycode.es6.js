 ...
 */
const basicToDigit = function(codePoint) ***REMOVED***
 ...
 */
const digitToBasic = function(digit, flag) ***REMOVED***
 ...
 * @private
 */
const adapt = function(delta, numPoints, firstTime) ***REMOVED***
	let k = 0;
 ...
 */
const decode = function(input) ***REMOVED***
	// Don't use UCS-2.
	const output = [];
	...
	***REMOVED***
	...
	for (let index = basic > 0 ? basic + 1 : 0; index < inputLength; /* no final expression */) ***REMOVED***
		...
		let oldi = i;
		for (let w = 1, k = base; /* no condition */; k += base) ***REMOVED***
 ...
 */
const encode = function(input) ***REMOVED***
	const output = [];
	...
	for (const currentValue of input) ***REMOVED***
		...
		for (const currentValue of input) ***REMOVED***
			...
			***REMOVED***
			if (currentValue == n) ***REMOVED***
				...
				let q = delta;
				for (let k = base; /* no condition */; k += base) ***REMOVED***
					const t = k <= bias ? tMin : (k >= bias + tMax ? tMax : k - bias);
					if (q < t) ***REMOVED***
						break;
					***REMOVED***
 ...
 * string.
 */
const toUnicode = function(input) ***REMOVED***
 ...
 * email address.
 */
const toASCII = function(input) ***REMOVED***
	return mapDomain(input, function(string) ***REMOVED***
...
export ***REMOVED*** ucs2decode, ucs2encode, decode, encode, toASCII, toUnicode ***REMOVED***;
