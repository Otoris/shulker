...
// Object properties:
typeCheck('***REMOVED***x: Number, y: Boolean***REMOVED***', ***REMOVED***x: 2, y: false***REMOVED***);             // true
typeCheck('***REMOVED***x: Number, y: Boolean***REMOVED***',       ***REMOVED***x: 2***REMOVED***);                 // false
typeCheck('***REMOVED***x: Number, y: Maybe Boolean***REMOVED***', ***REMOVED***x: 2***REMOVED***);                 // true
typeCheck('***REMOVED***x: Number, y: Boolean***REMOVED***',      ***REMOVED***x: 2, y: false, z: 3***REMOVED***);  // false
typeCheck('***REMOVED***x: Number, y: Boolean, ...***REMOVED***', ***REMOVED***x: 2, y: false, z: 3***REMOVED***);  // true

// A particular type AND object properties:
typeCheck('RegExp***REMOVED***source: String, ...***REMOVED***', /re/i);          // true
typeCheck('RegExp***REMOVED***source: String, ...***REMOVED***', ***REMOVED***source: 're'***REMOVED***); // false

// Custom types:
var opt = ***REMOVED***customTypes:
  ***REMOVED***Even: ***REMOVED*** typeOf: 'Number', validate: function(x) ***REMOVED*** return x % 2 === 0; ***REMOVED******REMOVED******REMOVED******REMOVED***;
typeCheck('Even', 2, opt); // true

// Nested:
var type = '***REMOVED***a: (String, [Number], ***REMOVED***y: Array, ...***REMOVED***), b: Error***REMOVED***message: String, ...***REMOVED******REMOVED***'
typeCheck(type, ***REMOVED***a: ['hi', [1, 2, 3], ***REMOVED***y: [1, 'ms']***REMOVED***], b: new Error('oh no')***REMOVED***); // true
```
...
##### example
```js
parseType('Number'); // [***REMOVED***type: 'Number'***REMOVED***]
```
### parsedTypeCheck(parsedType, input, options)
...
##### example
```js
parsedTypeCheck([***REMOVED***type: 'Number'***REMOVED***], 2); // true
...
* __Type__ = an `Identifier`, an `Identifier` followed by a `Structure`, just a `Structure`, or a wildcard `*` - eg. `String`, `Object***REMOVED***x: Number***REMOVED***`, `***REMOVED***x: Number***REMOVED***`, `Array***REMOVED***0: String, 1: Boolean, length: Number***REMOVED***`, `*`
...
* __Structure__ = `Fields`, or a `Tuple`, or an `Array` - eg. `***REMOVED***x: Number***REMOVED***`, `(String, Number)`, `[Date]`
* __Fields__ = a `***REMOVED***`, followed one or more `Field` separated by a comma `,` (trailing comma `,` is permitted), optionally an `...` (always preceded by a comma `,`), followed by a `***REMOVED***` - eg. `***REMOVED***x: Number, y: String***REMOVED***`, `***REMOVED***k: Function, ...***REMOVED***`
...
```js
***REMOVED******REMOVED***.toString.call(VALUE).slice(8, -1)
***REMOVED******REMOVED***.toString.call(true).slice(8, -1) // 'Boolean'
```
...
If used by itself, a 'fields' structure will pass with any type of object as long as it is an instance of `Object` and the properties pass - this allows for duck typing - eg. `***REMOVED***x: Boolean***REMOVED***`.

To check if the properties pass, and the value is of a certain type, you can specify the type - eg. `Error***REMOVED***message: String***REMOVED***`.

If you want to make a field optional, you can simply use `Maybe` - eg. `***REMOVED***x: Boolean, y: Maybe String***REMOVED***` will still pass if `y` is undefined (or null).

If you don't care if the value has properties beyond what you have specified, you can use the 'etc' operator `...` - eg. `***REMOVED***x: Boolean, ...***REMOVED***` will match an object with an `x` property that is a boolean, and with zero or more other properties.
